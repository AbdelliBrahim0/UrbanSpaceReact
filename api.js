// On récupère la variable d'environnement correctement selon l'outil
const API_BASE = (
  ...
  products: {
    ...
    getByCategory: async (categoryId, page = 1, limit = 12) => {
      const response = await fetchApi(
        `/public/categories/${categoryId}/products?page=${page}&limit=${limit}`
      );
      if (response.success && Array.isArray(response.items)) {
        return response;
      }
    ...
    getBySubcategory: async (subcategoryId, page = 1, limit = 12) => {
      const response = await fetchApi(
        `/public/subcategories/${subcategoryId}/products?page=${page}&limit=${limit}`
      );
      if (response.success && Array.isArray(response.items)) {
        return response;
      }
